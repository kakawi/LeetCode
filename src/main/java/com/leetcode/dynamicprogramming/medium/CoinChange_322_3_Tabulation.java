...
/**
 * dp O(ns) pseudo polynomial. From #1, we can see that f(x)=min(1+f(x-ci)) Idea: using array of all
 * possible amounts - [0, amount]
 */
public class CoinChange_322_3_Tabulation implements CoinChange_322 {
  ...
  public int coinChange(int[] coins, int amount) {
    ...
    if (allAmounts[amount] == Integer.MAX_VALUE) {
      return -1;
    }
